...
class DataMessageResponse extends FormatInterface {
  ...
  constructor(data) {
    ...
    super(data, protons__default['default'](proto$5).PeernetDataMessageResponse, {name});
  }
}
...
const getAddress = async () => {
  ...
  if (now - address.timestamp > 300) {
    address.value = await fetch$1__default['default']('https://ipv6.icanhazip.com/');
...
class DhtEarth {
  ...
  async getCoordinates(provider) {
    ...
    let response = await fetch$1__default['default'](request);
...
const hasDaemon = async () => {
  try {
    let response = await fetch$1__default['default']('http://127.0.0.1:1000/api/version');
    response = await response.json();
